...
from kivymd.list import OneLineRightIconListItem,OneLineLeftRightIconListItem
...
<NavigationDrawerButton>:
    ...
    BoxLayout:
        id: _right_container
        size_hint: None, None
        x: root.x + root.width - _badge.texture_size[0] - dp(12) #  - m_res.HORIZ_MARGINS
...
<NavigationDrawerRightIconButton>:
    ...
    BoxLayout:
        id: _left_container
        size_hint: None, None
        x: root.x + root.width - _badge.texture_size[0] - dp(36) #  - m_res.HORIZ_MARGINS
        y: root.y + root.height/2 - self.height/2
        size: root.width, root.height
        NDBadgeLabel:
            id: _badge
...
<NavigationDrawerIconButton>:
    ...
    BoxLayout:
        id: _right_container
        size_hint: None, None
        x: root.x + root.width - _badge.texture_size[0] - dp(72) #  - m_res.HORIZ_MARGINS
        y: root.y + root.height/2 - self.height/2
        size: dp(72), root.height
        NDBadgeLabel:
            id: _badge
            theme_text_color: 'Secondary' if not root._active else 'Custom' if root.use_active else 'Custom'
            text_color: root.theme_cls.secondary_text_color if not root._active else root.active_color if \
...
class NavigationDrawerLeftRightIconButton(OneLineLeftRightIconListItem):
    '''
    An item in the :class:`MDNavigationDrawer`.
    '''
    _active = BooleanProperty(False)
    _active_color = ListProperty()
    _lefticon = ObjectProperty()
    _righticon = ObjectProperty()
    divider = None
    ...
    def on_left_icon(self, instance, value):
        self.ids['_lefticon'].text = u"{}".format(md_icons[value])

    def on_right_icon(self, instance, value):
        self.ids['_righticon'].text = u"{}".format(md_icons[value])

    def on_active_color_type(self, *args):
        self._set_active_color(args)


class NavigationDrawerRightIconButton(OneLineRightIconListItem):
    _active = BooleanProperty(False)
    _active_color = ListProperty()
    _icon = ObjectProperty()
    divider = None
    ...
    use_active = BooleanProperty(True)
    def __init__(self, **kwargs):
        super(NavigationDrawerRightIconButton, self).__init__(**kwargs)
        self._set_active_color()
        self.theme_cls.bind(primary_color=self._set_active_color_primary, accent_color=self._set_active_color_accent)
        Clock.schedule_once(lambda x: self.on_icon(self, self.icon))
